...
void Screenshot::take()
{
    ...
    QSize newSize = QSize(ceil(m_image.width() / (double)m_blockWidth) * m_blockWidth,ceil(m_image.height() / (double)m_blockWidth) * m_blockWidth);
    if(m_image.size() != newSize){
        ...
        for (int j=0; j < newSize.height();j++){
            for (int i=0; i < newSize.width();i++){
                newImage.setPixel(i,j,m_image.pixel(i<m_image.width()? i:m_image.width()-1,j<m_image.height()?j:m_image.height()-1));
            }
        }
        m_image = newImage;
    }
...
QImage Screenshot::getImage() const
{
    return m_image;
}
