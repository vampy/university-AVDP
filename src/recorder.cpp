...
Recorder::Recorder(QObject *parent,int fps) : QObject(parent),m_fps(fps), m_queue(new QQueue<QImage>),m_screenshot(new Screenshot),
    m_timer(new QTimer(this)),m_workerTimer(new QTimer(this))
{
        ...
        if(m_currentFrame.isNull()){
            ...
            for(int y=0;y<nextFrame.height();y+=m_blockWidth){
                for(int x=0;x<nextFrame.width();x+=m_blockWidth){
                    Imageblock currentImageBlock(m_currentFrameId,QPoint(x,y),m_currentFrame.copy(x,y,m_blockWidth,m_blockWidth));
                    Imageblock nextImageBlock(m_currentFrameId+1,QPoint(x,y),nextFrame.copy(x,y,m_blockWidth,m_blockWidth));
            ...
            if(m_currentFrameId%25==0){
                qDebug()<<"We have "<<counter<< "/"<< (m_currentFrame.width()/m_blockWidth) *(m_currentFrame.height()/m_blockWidth)<<"equal blocks in frame"<<m_currentFrameId;
            }
